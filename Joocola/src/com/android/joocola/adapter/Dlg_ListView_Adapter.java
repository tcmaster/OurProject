...
public class Dlg_ListView_Adapter<T> extends BaseAdapter {
	 ...
	 * 根据字符串确定pos
	 */
	public void setPos(String keyword) {
		for (int i = 0; i < baseDataInfos.size(); i++) {
			if (((Map<String, String>) baseDataInfos.get(i)).get("name")
					.equals(keyword)) {
				setPos(i);
			}
		}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (baseDataInfos.get(position) instanceof BaseDataInfo) {
			...
			holder.tv_text.setText(((Map<String, String>) baseDataInfos
