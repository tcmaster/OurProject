	...
	public class MultiChatListener implements PacketListener {
		...
		public void processPacket(Packet arg0) {
			...
			DelayInformation delay = (DelayInformation) message.getExtension(
					"x", "jabber:x:delay");
			MultiChatInfo info = new MultiChatInfo();
			info.setContent(message.getBody());
			info.setIsFrom(arg0.getFrom().split("/")[1].split("@")[0]);
			info.setIsTo(arg0.getTo().split("@")[0]);
			if (delay != null)
				info.setTime(Utils.formatDate(delay.getStamp()));
			else
				info.setTime(Utils.formatDate(new Date(System
						.currentTimeMillis())));
			info.setMultikey(key);
			info.setIsRead(0);
			info.setImgUrl("");
			info.setUser("u" + JoocolaApplication.getInstance().getPID());
			Log.e("lixiaosong", info.toString());
			Log.e("lixiaosong", arg0.toXML());
			try {
				JoocolaApplication.getInstance().getDB().save(info);
			} catch (DbException e) {
				e.printStackTrace();
			}
			JoocolaApplication.getInstance().sendBroadcast(
					new Intent(Constans.MULTI_CHAT_ACTION));
		}
	}
}
