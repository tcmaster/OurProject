...
public class XMPPChat {
	...
	public static void stopEveryThing() {
		try {
			JoocolaApplication.getInstance().getDB()
					.deleteAll(MultiChatInfo.class);
		} catch (DbException e) {
			e.printStackTrace();
		}
		if (chatService != null)
			chatService.stopHeartService(JoocolaApplication.getInstance());
	}
			...
			public void fileTransferRequest(FileTransferRequest arg0) {
				...
				if (Environment.getExternalStorageState().equals(
					...
					try {
						...
						info.setIsFrom(arg0.getRequestor().split("@")[0]);
						info.setIsTo(connection.getUser().split("@")[0]);
						info.setIsRead(0);
						...
						context.sendBroadcast(new Intent(Constans.CHAT_ACTION));
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else {
